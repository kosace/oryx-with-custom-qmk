...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [0] = LAYOUT_voyager(
    ...
    KC_ESCAPE,      KC_Q,           KC_W,           KC_E,           KC_R,           KC_T,                                           DE_Z,           KC_U,           KC_I,           KC_O,           KC_P,           DE_DQOT,        
    KC_LEFT_SHIFT,  KC_A,           KC_S,           KC_D,           KC_F,           KC_G,                                           KC_H,           KC_J,           KC_K,           KC_L,           DE_SCLN,        KC_RIGHT_SHIFT, 
    MO(3),          DE_Y,           KC_X,           KC_C,           KC_V,           KC_B,                                           KC_N,           KC_M,           KC_COMMA,       KC_DOT,         DE_MINS,        DUAL_FUNC_0,    
  ...
  [2] = LAYOUT_voyager(
    KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT,                                 KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, 
    DE_EQL,         DE_EXLM,        DE_AT,          DE_HASH,        DE_DLR,         DE_PERC,                                        ST_MACRO_0,     DE_AMPR,        DE_ASTR,        DE_LPRN,        DE_RPRN,        KC_TRANSPARENT, 
    KC_DELETE,      DE_AE,          DE_SS,          DE_UE,          DE_OE,          CW_TOGG,                                        KC_LEFT,        KC_DOWN,        KC_UP,          KC_RIGHT,       DE_TILD,        KC_TRANSPARENT, 
    KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, DE_LBRC,        DE_LCBR,        DE_PLUS,                                        ST_MACRO_1,     DE_RCBR,        DE_RBRC,        KC_TRANSPARENT, DE_PIPE,        DE_BSLS,        
                                                    KC_TRANSPARENT, KC_TRANSPARENT,                                 KC_TRANSPARENT, KC_TRANSPARENT
  ),
  [3] = LAYOUT_voyager(
    ...
    KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT, KC_TRANSPARENT,                                 KC_AUDIO_VOL_UP,KC_7,           KC_8,           KC_9,           LGUI(LSFT(KC_S)),KC_TRANSPARENT, 
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
    ...
    case ST_MACRO_1:
    if (record->event.pressed) {
      SEND_STRING(SS_TAP(X_EQUAL)SS_DELAY(1)  SS_TAP(X_SPACE));
    }
    break;
...
const key_override_t quot_key_override = ko_make_basic(MOD_MASK_SHIFT, DE_DQOT, KC_BACKSLASH);

// mac 
const key_override_t copy_override = ko_make_with_layers(MOD_MASK_CTRL, KC_C, LGUI(KC_C), 1 << 1);
const key_override_t paste_override = ko_make_with_layers(MOD_MASK_CTRL, KC_V, LGUI(KC_V), 1 << 1);
const key_override_t cut_override = ko_make_with_layers(MOD_MASK_CTRL, KC_X, LGUI(KC_X), 1 << 1);
const key_override_t select_override = ko_make_with_layers(MOD_MASK_CTRL, KC_A, LGUI(KC_A), 1 << 1);
const key_override_t find_override = ko_make_with_layers(MOD_MASK_CTRL, KC_F, LGUI(KC_F), 1 << 1);
const key_override_t undo_override = ko_make_with_layers(MOD_MASK_CTRL, DE_Z, LGUI(DE_Z), 1 << 1);
const key_override_t redo_override = ko_make_with_layers(MOD_MASK_CTRL, DE_Y, S(LGUI(DE_Z)), 1 << 1);
const key_override_t save_override = ko_make_with_layers(MOD_MASK_CTRL, KC_S, LGUI(KC_S), 1 << 1);
